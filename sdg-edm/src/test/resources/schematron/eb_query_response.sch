...
<sch:schema 
    ...
    <sch:pattern>
        ...
        <sch:rule context="query:QueryResponse[
            ...
            <sch:assert test="count(rim:RegistryObjectList) = 0" id="R-EB-REQ-S009"
                >An unsuccessful OOTS response does not include a RegistryObjectList</sch:assert>
            <sch:assert test="count(rs:Exception) = 1" id="R-EB-REQ-S010"
    ...
    <sch:pattern>
        <sch:rule context="rim:Slot[@name='Requirement']/rim:SlotValue/sdg:Requirement">          
            <sch:assert test="count(sdg:EvidenceTypeList)=0" id="R-EB-REQ-S016"
                >The xs:element name="Requirement" MUST not contain the EvidenceTypeList.</sch:assert>      
        </sch:rule>
    </sch:pattern>
